...
Defines how the renter's ETF portfolio is computed in the buy-vs-rent simulation. The ETF lives in USD throughout (matching the underlying instrument, e.g. CSPX.L). Two configurable mechanisms drive growth: a one-time seed of cash at month 0 (the cash that would have been used for down payment + closing, converted to USD), and monthly contributions in USD whose amount depends on the selected mode. A monthly COP/USD rate evolves from a user-set initial value at a user-set annual devaluation, compounded monthly.
...
- GIVEN `invertirCashInicial=true` and `cashInicial=39_000_000`, `tasaCopUsdInicial=4200`
